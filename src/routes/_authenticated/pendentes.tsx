...
export const Route = createFileRoute("/_authenticated/pendentes")({
  head: () => ({
    meta: [{ title: "Pendentes de Conclusão — HelpDesk Buritis" }],
  }),
  component: Pendentes,
});
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-priority-media/15 text-priority-media">
          <ClipboardList className="h-5 w-5" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-foreground">Pendentes de Conclusão</h1>
          <p className="text-sm text-muted-foreground">
            {filtered.length} chamado(s) parcialmente completados
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-2 rounded-xl border bg-card p-12 text-center text-muted-foreground">
          ...
          <p className="text-sm">Nenhum chamado pendente de conclusão.</p>
        </div>
      ) : (
