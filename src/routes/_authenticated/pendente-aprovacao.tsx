...
export const Route = createFileRoute("/_authenticated/pendente-aprovacao")({
  head: () => ({
    meta: [{ title: "Pendente de Aprovação — HelpDesk Buritis" }],
  }),
  component: PendenteAprovacao,
});

function PendenteAprovacao() {
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {filtered.length} baixa(s) aguardando aprovação do administrador
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-2 rounded-xl glass-card p-12 text-center text-muted-foreground">
          ...
          <p className="text-sm">Nenhuma baixa pendente de aprovação.</p>
        </div>
      ) : (
